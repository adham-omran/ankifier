...
- =source-name= — The org file keyword to read the value from (e.g. "SOURCE_NAME",
  read from a =#+SOURCE_NAME:= line in the org file).
- =source-type= — How to retrieve the value. Currently supported: =file= (reads
  from an org file keyword).
