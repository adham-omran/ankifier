...
A custom field can be added with the flag TODO

What if I want a number of fields? A list?

Should the source be a variable? Or inferred from the buffer?
Something like #+SOURCE_FIELD? Or a property? :SOURCE:?

Maybe have a map structure where there are two keys
- field-name
  Anki template field name

- field-source

  Where to get the field's value, property or variable
- field-value

  Grab the value based on ~field-source~.
*** How Should The User Add Fields?
