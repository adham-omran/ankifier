...
+ =ankifier-create-from-region=: Parse basic and cloze questions in the active
  region.
+ =ankifier-find-to-be-ankified=: Interactively search the buffer for
  un-ankified questions and prompt to ankify them.
...
+ Set =ankifier-arabic= to =t= to use Arabic question marks (؟) instead of
  Latin (?).
...
- =source-type= — How to retrieve the value:
  - =file= — reads from an org file keyword (e.g. =#+SOURCE_NAME: value=). All
    cards in the file get the same value.
  - =property= — reads from an org property drawer (e.g. =:SOURCE_NAME: value=).
    Supports inheritance, so different headings can have different values.
...
Cards created under "Chapter 1" will have =Source: Introduction to Algorithms, Ch. 1=,
